...
class PaymentCheckActivity: AppCompatActivity() {
    ...
    fun calculateTotal(cart:ArrayList<MenuData>): Int{
        ...
        for(i in cart){//총합계 출력
            ...
            qrString+= i.category + ":" + i.UID + ":" + i.select_count.toString() + "//"
        }
        Log.d("cart", qrString)
    ...
    fun cartToQr(cart:ArrayList<MenuData>): String{
        refreshCart(cartManager,cartAdapter)
        qrString = ""
        for(i in cart){//총합계 출력
            qrString+= i.UID + ":" + i.imageURL + ":" + i.name + ":" + i.price + ":" + i.quantity + ":" + i.serving + ":"  +i.select_count.toString() + ":" + i.category + "//"
        }
        Log.d("cart", qrString)
        return qrString
    }
